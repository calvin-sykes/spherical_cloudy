...
def get_halo(virialm,redshift,gastemp):
	# Begin the timer
	timeA = time.time()
	...
	if loadprev and virialm != 1.0E7:
		...
		infname = "data/Burkert_model_mass{0:s}_redshift{1:s}.npy".format(mstring,rstring)
	...
	while (critHI or critHeI or critHeII) or (iteration <= miniter):
		...
		"""
		densitym = calc_Jnur.HEcalc_Burkert(radius/rscale,prof_temperature,masspp,rhods,rscale,0.01,densitym[0],barymass)
		...
		#sys.exit()
		"""
#		prof_pressure = calc_Jnur.pressure_Burkertgas(prof_temperature, fgas, radius, masspp, rhods, rscale)
		prof_pressure = calc_Jnur.pressure_Burkert(prof_temperature, radius, masspp, rhods, rscale)
		#############
		# TEST PLOT #
		#############
		#print "pressure!"
...
#		prof_pressure = cen_pressure * kB * temp_pressure
		...
		print "Integrating over all directions"
	...
	rstring = ("{0:3.2f}".format(redshift)).replace(".","p").replace("+","")
	outfname = "data/Burkert_model_mass{0:s}_redshift{1:s}".format(mstring,rstring)
